...
export default class AGCGame extends React.Component {
  ...
  switchCurrentNode(option) {
    ...
    if (option.modifier) {
      if (option.modifier === "drunk") {
        nodes[nodes.indexOf(this.findNode("11"))].natText =
          "You somehow manage to get an audience with the king, but your drunkness offends him. He orders the guards to throw you in jail where you shall spend the rest of your days.";
        ...
        nodes[nodes.indexOf(this.findNode("2"))].natText =
        ...
        nodes[nodes.indexOf(this.findNode("22"))].natText =
  ...
  render() {
    return (
      <AGRender
        node={this.findNode(this.state.currentNode)}
        text={this.state.log}
        onClick={this.switchCurrentNode}
      />
    );
  }
}
